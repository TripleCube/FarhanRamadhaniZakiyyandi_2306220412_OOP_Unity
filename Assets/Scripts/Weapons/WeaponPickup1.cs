...
{
    [SerializeField] Weapon1 weaponHolder;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            Weapon1 playerWeapon = other.gameObject.GetComponentInChildren<Weapon1>();
    ...
    void TurnVisual(bool on)
    {
        if (on)
        {
            ...
            weapon.GetComponent<Weapon1>().enabled = true;
        }
        else
        {
            weapon.GetComponent<SpriteRenderer>().enabled = false;
            weapon.GetComponent<Animator>().enabled = false;
            weapon.GetComponent<Weapon1>().enabled = false;
        }

    }

    void TurnVisual(bool on, Weapon1 weapon)
    {
        if (on)
        {
            weapon.GetComponent<SpriteRenderer>().enabled = true;
